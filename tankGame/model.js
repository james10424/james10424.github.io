...
class Player extends GameObject {
    ...
    *dash() {
        ...
        var _mouse = camera.screenToWorldXY(mouseX, mouseY);
        // var dashDirection = this.position.angleRad(mouse);
        var dashDirection = mobile ? degToRad(euler.yaw) : this.position.angleRad(_mouse);
    ...
    draw() {
        ...
        */
        ...
        if (this.isBuild) {
            var _mouse = camera.screenToWorldXY(mouseX, mouseY);
            var pos = rotateCCWAboutTrig(this.position.x, this.position.y, this.position.x + this.radius / 2 + this.buildWidth * 2 + this.buildDist, this.position.y + this.buildLength / 2, Math.sin(this.position.angleRad(_mouse)), Math.cos(this.position.angleRad(_mouse)));
            // var pos = this.position;
            var rotation = this.position.angleRad(_mouse);
        ...
        if (LMB) {
            if (this.isBuild) {
                if (this.canBuild()) {
                    ...
                    var _mouse = camera.screenToWorldXY(mouseX, mouseY);
                    var pos = rotateCCWAboutTrig(this.position.x, this.position.y, this.position.x + this.radius / 2 + this.buildWidth * 2 + this.buildDist, this.position.y, Math.sin(this.position.angleRad(_mouse)), Math.cos(this.position.angleRad(_mouse)));
                    // var pos = this.position;
                    var rotation = this.position.angleRad(_mouse);
...
class Laser {
    ...
    attack() {
        ...
        var rot = mobile ? degToRad(euler.yaw) : this.from.position.angleRad(_mouse);
